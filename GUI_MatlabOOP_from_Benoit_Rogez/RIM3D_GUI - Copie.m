...
set(BouttonXY_Up,'ButtonPushedFcn',@(src,event) PiezoMoveXY(Piezo,'X',StepXYValue.Value,1,XPosition,YPosition));
...
set(BouttonXY_Left,'ButtonPushedFcn',@(src,event) PiezoMoveXY(Piezo,'Y',StepXYValue.Value,-1,XPosition,YPosition));
...
set(BouttonXY_Right,'ButtonPushedFcn',@(src,event) PiezoMoveXY(Piezo,'Y',StepXYValue.Value,1,XPosition,YPosition));
...
set(BouttonXY_Down,'ButtonPushedFcn',@(src,event) PiezoMoveXY(Piezo,'X',StepXYValue.Value,-1,XPosition,YPosition));
...
StepZValue.Value = 100; StepZValue.FontWeight = 'bold'; StepZValue.FontSize = TextFontSize;
...
set(BouttonZ_Up,'ButtonPushedFcn',@(src,event) PiezoMoveZ(Piezo,'Z',StepZValue.Value,1,ZPosition));
...
set(BouttonZ_Down,'ButtonPushedFcn',@(src,event) PiezoMoveZ(Piezo,'Z',StepZValue.Value,-1,ZPosition));
...
DiffuserVelocityValue.ValueDisplayFormat = '%3.3f';
DiffuserVelocityValue.Value = 100; DiffuserVelocityValue.FontWeight = 'bold'; DiffuserVelocityValue.FontSize = TextFontSize;
...
set(DiffuserRotateButton,'ValueChangedFcn',@(src,event) DiffuserRotate(Diffuser));
...
ROICentered.Value = 1;
set(ROICentered,'ValueChangedFcn',@(src,event) SetCameraROI(Camera,ROICentered.Value,ROILeftValue.Value,ROITopValue.Value,ROIWidthValue.Value,ROIHeigthValue.Value,CameraImage));
...
set(ROIWidthValue,'ValueChangedFcn',@(src,event) SetCameraROI(Camera,ROICentered.Value,ROILeftValue.Value,ROITopValue.Value,ROIWidthValue.Value,ROIHeigthValue.Value,CameraImage));
...
set(ROILeftValue,'ValueChangedFcn',@(src,event) SetCameraROI(Camera,ROICentered.Value,ROILeftValue.Value,ROITopValue.Value,ROIWidthValue.Value,ROIHeigthValue.Value,CameraImage));
...
set(ROIHeightValue,'ValueChangedFcn',@(src,event) SetCameraROI(Camera,ROICentered.Value,ROILeftValue.Value,ROITopValue.Value,ROIWidthValue.Value,ROIHeigthValue.Value,CameraImage));
...
set(ROITopValue,'ValueChangedFcn',@(src,event) SetCameraROI(Camera,ROICentered.Value,ROILeftValue.Value,ROITopValue.Value,ROIWidthValue.Value,ROIHeigthValue.Value,CameraImage));
...
AcquisitionPanelHeight = 6*TextHeight;
...
NumberSpeckleValue.Limits = [60 inf]; NumberSpeckleValue.LowerLimitInclusive = 'on';
...
set(StartAcquisitionButton,'ButtonPushedFcn',@(src,event) StartAcquisition());
...
Main.UserData{2} = ObjectiveName.Value;
Main.UserData{3} = UserName.Value;
...
function PiezoMoveXY(Piezo,Axe,StepValue,Direction,DisplayX,DisplayY)
...
function PiezoMoveZ(Piezo,Axe,StepValue,Direction,DisplayZ)
    % input: Piezo (class), Axes (string), StepSize (number), Direction (1 or -1), Displays for current position.
    Piezo.Move(Axe,'Relative',StepValue*Direction);
    % Nouvelle fonction Move doit contenir l'axe
    % 
...
function DiffuserRotate(Diffuser)
...
function CameraSaveLast(Camera)
    SaveName = uigetdir(Camera.SavePath);
...
function StartAcquisition(Camera, Lens, LensMin, LensMax, LensStep, Diffuser, NumberSpeckles, SaveFolder)
    ...
    DiffuserAngle = 360/NumberSpeckles;
   
    % Save a txt file with experimental parameters
    fileID = fopen(fullfile(Camera.SaveFolder,'ExperimentData.txt'),'w');
    fprintf(fileID,'Date of Experiment: %s', datetime('now','Format','yyyy-MM-dd'));
    fprintf(fileID,'Sample: %s', Main.UserData{1});
    fprintf('Experimentalist: %s', Main.UserData{2});
    fprintf('Objective: %s', Main.UserData{3});
    fprintf('CAMERA_PARAMETERS');
    fprintf('Exposure_Time(ms): %f', Camera.ExposureTime);
    fprintf('Image_Size_(pixels): %f, %f', Camera.ROIWidth, Camera.ROIHeight); 
    fprintf('Readout_Mode: %s', Camera.ReadoutMode);
    fprintf('LENS_PARAMETERS');
    fprintf('Focal Power Range (Dioptries): %f to %f', LensMin, LensMax);
    fprintf('Focal Power Step (Dioptries): %f', LensStep);
    fprintf('SPECKLE_PARAMETERS')
    fprintf('Number of Speckles: %f', NumberSpeckles);
    fclose(fileID);
    ...
    for i = 1:NumberSpeckles
        ...
        for j = 1:NStepsLens
            ...
            Camera.Capture1more();
            % Set savename
            SaveName = strcat('Speckle',num2str(i,'%03d'),'Lens',num2str(100*(LensPower+1),'%03.0f'));
