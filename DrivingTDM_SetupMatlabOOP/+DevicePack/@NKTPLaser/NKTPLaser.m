classdef NKTPLaser < handle
    ...
    properties(Constant = true)
        ...
        addressModuleTypeFilter = 16;  % See the address in the SuperK Varia module( in this case the 
    ...
    properties (Access = private)
        portName;     %% The COM port where the modules are conneced
        powerLevel = 0;    %% current level of laser power(initial value is set to zero) 
        waveLenght = 475;  %% the wavelength of the LASER output
        bandWidth  = 10;   %% spectral bandwidth (initial value is set to 10nm)
    ...
    methods
        function obj = NKTPLaser()
            ...
            %%% Find the all possible COM Ports 
            ...
            for ii = 1:size(portsActive, 2)
                port_number = 20;  %% Assumptions that it will be maximum number of ports (COM port)
